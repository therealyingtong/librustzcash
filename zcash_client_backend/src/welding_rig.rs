...
fn scan_output<P: consensus::Parameters>(
    ...
    for (account, ivk) in ivks.iter().enumerate() {
        let (note, to) = match try_sapling_compact_note_decryption(parameters, height, ivk, &epk, &cmu, &ct) {
            Some(ret) => ret,
            None => continue,
        };
...
mod tests {
    ...
    fn scan_block_with_my_tx() {
        ...
        let txs = scan_block(&consensus::MainNetwork, cb, &[extfvk], &[], &mut tree, &mut []);
        assert_eq!(txs.len(), 1);
    ...
    fn scan_block_with_my_spend() {
        ...
        let mut tree = CommitmentTree::new();
        let txs = scan_block(&consensus::MainNetwork, cb, &[], &[(&nf, account)], &mut tree, &mut []);
        assert_eq!(txs.len(), 1);
